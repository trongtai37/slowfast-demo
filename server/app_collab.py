...
def index2class():
    file_path = '/content/SlowFast/ava/ava_action_list_v2.0.csv'
...
def get_test_input(input_dim, CUDA):
    img = cv2.imread("/content/SlowFast/imgs/dog.jpg")
...
def imshow(video_name, frame, bboxes, labels, probs, ids, count):
    for bbox, lables, prob, i in zip(bboxes, labels, probs, ids):
        ...
        for lable, p in zip(lables, prob):
            ...
            cv2.rectangle(frame, (real_x_min, real_y_min), (real_x_max,
                          real_y_max), (255, 87, 51), 2)  # yellow color

            cv2.putText(frame, index2class()[str(lable)].split("(")[0] + ':' + str(round(p, 2)),
                        (real_x_min + 15, real_y_max - 15 * count_2),
                        cv2.FONT_HERSHEY_COMPLEX,
                        0.5, (0, 255, 195), 1, False)
        ...
        cv2.imwrite(
            f"demo/outputs/{video_name}/frames/%04d.jpg" % count, frame)
...
def arg_parse():
    ...
    parser.add_argument("--video", dest='video', help="Video to run detection upon",
                        default="/content/SlowFast/new.mp4", type=str)
    ...
    parser.add_argument("--cfg", dest='cfgfile', help="Config file",
                        default="/content/SlowFast/cfg/yolov3.cfg", type=str)

    # change the path accordingly
    parser.add_argument("--weights", dest='weightsfile', help="weightsfile",
                        default="/content/SlowFast/yolov3.weights", type=str)
...
model = Darknet("/content/SlowFast/cfg/yolov3.cfg")
model.load_weights("/content/SlowFast/yolov3.weights")
print("Network successfully loaded")

# change the path accordingly
print("load deep sort network....")
deepsort = DeepSort("/content/SlowFast/deep/checkpoint/ckpt.t7")
...
path_to_checkpoint = "/content/SlowFast/slowfast_weight.pth"
...
video_path = "/content/SlowFast/demo/inputs/walk.mp4"
video_name = "walk"
os.system(f"mkdir -p demo/outputs/{video_name}/frames")
os.system(f"mkdir -p demo/outputs/{video_name}/videos")
SlowFast(model, CUDA, video_path, video_name)
...
UPLOAD_FOLDER = '/content/SlowFast/demo/inputs/'
...
def index():
    ...
    if video and allowed_file(video.filename):
        ...
        video.save(image_path)
        video_name = str(time.time())
        os.system(f"mkdir -p demo/outputs/{video_name}/frames")
        os.system(f"mkdir -p demo/outputs/{video_name}/videos")
        SlowFast(model, CUDA, video_path, video_name)
        video_output_path = f"demo/outputs/{video_name}/videos/video.mp4"
        return {
